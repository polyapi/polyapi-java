...
export class JobsStrategy extends ConfigVariableStrategy<Jobs> {
  getOneFromList(configVariables: ConfigVariable[]): ConfigVariable | null {
    ...
    for (const configVariable of sortedConfigVariables) {
      ...
      if (currentVariable.value.minimumIntervalTimePerExecution > lastVariable.value.minimumIntervalTimePerExecution) {
        value = {
          ...value,
  ...
  private validateValue(newValue: Jobs, configVariables: ConfigVariable[], foundVariable: ConfigVariable | undefined) {
    ...
    for (const configVariable of configVariables) {
      ...
      if (newValue.minimumIntervalTimePerExecution < parsedConfigVariable.minimumIntervalTimePerExecution) {
        throw new BadRequestException(`Current job limit (${newValue.minimumIntervalTimePerExecution} minutes) cannot be lower than ${parsedConfigVariable.minimumIntervalTimePerExecution} minutes.`);
      }
    }
  ...
  async configure(name: string, value: unknown, tenantId: string | null, environmentId: string | null): Promise<ConfigVariable> {
    ...
    if (newValue.minimumIntervalTimePerExecution < 5) {
